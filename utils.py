...
class MyClassifier(oa.Classifier):
    def __init__(self, model, tokenizer, batch_size=1, max_len=64, device='cpu', plain_model=False):
        self.model = model
    ...
    def get_prob(self, texts):
        ...
        preds = get_preds(self.model, data_iter, plain_model=self.plain_model)
        return preds
...
def cal_true_success_rate(advs, dataset):
    ...
    for i, adv in enumerate(advs):
        ...
        pred_gens.append(np.argmax(pred_gen) if gen else pred_org)
...
def evaluate_alternating_batch_single(model, batch, allow_grad=False, preds_only=False):
    
    preds_prob = []
    return preds_prob

def get_preds(model, val_iter, plain_model=False):
    model.eval()
    preds = []
    for batch in val_iter:
        if plain_model:
            ...
            seq = batch['input_ids']
            attn_masks = batch['attention_mask']
            logits = model(seq, attn_masks).logits
