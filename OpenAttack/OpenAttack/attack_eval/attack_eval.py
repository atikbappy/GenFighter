...
class AttackEval:
    ...
    def __iter_metrics(self, iterable_result):
        for data, result in iterable_result:
            adversarial_sample, attack_time, invoke_times, ori_y_prob, adv_y_prob = result
            ret = {
                "data": data,
                "success": adversarial_sample is not None,
                "result": adversarial_sample,
                'ori_y_prob': ori_y_prob,
                'adv_y_prob': adv_y_prob,
                "metrics": {
    ...
    def eval(self, dataset: Iterable[Dict[str, Any]], total_len : Optional[int] = None, visualize : bool = False, progress_bar : bool = False, num_workers : int = 0, chunk_size : Optional[int] = None):
        ...
        for i, res in enumerate(result_iterator):
            ...
            if visualize and (TAG_Classification in self.victim.TAGS):
                x_orig = res["data"]["x"]
                ori_y_prob, adv_y_prob = res['ori_y_prob'], res['adv_y_prob']

                if ori_y_prob is None: # When exception occurs res['ori_y_prob'] becomes None
                    ori_y_prob = self.victim.get_prob([x_orig])[0] 
                ...
                if res["success"]:
                    ...
                    if Tag("get_prob", "victim") in self.victim.TAGS:
                        ...
                        try:
                            y_orig, y_adv = ori_y_prob, adv_y_prob
                        finally:
                        ...
                        self.victim.set_context(res["data"], None)
                        try:
                            y_orig, y_adv = ori_y_prob.argmax().item(), adv_y_prob.argmax().item()
                        finally:
                            self.victim.clear_context()
                    else:
                ...
                ori_y_label = res["data"]["y"]
                save_results.append([x_orig, y_orig, x_adv, y_adv, info, ori_y_label])
